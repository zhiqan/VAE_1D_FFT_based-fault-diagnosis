...
data_dir='D:\\meta-GMVAE_故障诊断\\PU\\SHUJU'
N = 150  # total num instances per class
K_mtr = 100  # total num meta_train classes
K_mva = 28  # total num meta_val classes
K_mte = 28  # total num meta_test classes

x_mtr = np.load(os.path.join(data_dir, 'train_FFT1024.npy'))

x_mtr = np.reshape(x_mtr, [100*150, 1024])

y_tr=np.ones([100*150,1]) #设置的标签没有意义

x_mva = np.load(os.path.join(data_dir, 'test_FFT1024.npy'))

x_mva = np.reshape(x_mva, [28*150,1024])

y_te=np.ones([28*150,1]) #设置的标签没有意义
